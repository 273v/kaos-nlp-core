...
These standards apply to Python code in `kaos-nlp-core`.

`kaos-nlp-core` publishes the `kaos_nlp_core` import package. This package publishes CLI entry point(s): `kaos-nlp`, `kaos-nlp-serve`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-nlp-core` publishes import package `kaos_nlp_core`.
- Declare the public API in `kaos_nlp_core.__all__` when the package has
...
- Names exported from `kaos_nlp_core.__all__`.
