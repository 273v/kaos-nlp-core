...
def most_similar(
    ...
    *,
    ...
    """Return the top-``k`` most-similar candidates from ``choices``.

    Args:
        query: Reference string.
        choices: Candidate strings to rank.
        algorithm: Metric name. Same keys as :func:`compare_batch` —
            common picks: ``"jaro-winkler"`` (default), ``"levenshtein"``,
            ``"token-jaccard"``, ``"ngram-cosine"``.
        k: How many candidates to return.
        n: N-gram size for n-gram metrics.
        lowercase: Lowercase tokens for token-level metrics.
    ...
    Returns:
        ``[(choice, similarity), ...]`` ordered by descending similarity.
        Ties are broken by the original ``choices`` index for stability.
    """
    return _raw.most_similar(
        query,
        list(choices),
        algorithm,
        k,
...
def least_similar(
    ...
    *,
    ...
    """Return the top-``k`` *least*-similar candidates from ``choices``.

    Same arguments as :func:`most_similar`, except ``threshold`` is a
    *ceiling*: candidates whose similarity exceeds it are dropped.

    Returns ``[(choice, similarity), ...]`` ordered by ascending similarity.
    """
    return _raw.least_similar(
        query,
        list(choices),
        algorithm,
        k,
