...
def test_bench_cosine_one_to_many_grid() -> None:
    ...
    is correctness — `|rust - numpy| < 1e-5` on every cell. The
    speedup-vs-numpy picture is honestly mixed:

    - **Rust wins big** (5-25x) on `dim ≥ 768`, `n_rows ≥ 1000` —
      NumKong's AVX-512 + compensated arithmetic outpaces numpy's
      BLAS at the same shape.
    - **Numpy wins** on small `dim=256` or `n_rows ≤ 100` —
      the PyO3 boundary overhead (~5-10 µs/call for array
      conversion + GIL handoff) dominates when the math itself is
      microseconds-fast.
    - **Roughly tied** in the middle.

    The JSON artifact is the source of truth for tracking drift
    across releases. We do NOT assert speedup ≥ 1x because that
    fails on the small-corpus cells where numpy's well-tuned BLAS
    is already so fast that any boundary overhead loses.
    """
