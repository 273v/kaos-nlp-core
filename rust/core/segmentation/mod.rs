...
// ─── Embedded default Punkt model ───────────────────────────────────────────

/// Default Punkt model (~12 MB gzipped JSON) baked into the crate via
/// `include_bytes!` at build time. End users (Rust callers and the PyO3
/// binding alike) get a working sentence tokenizer with no filesystem
/// state. The wheel ships only the .so containing these bytes; the file
/// itself ships in the sdist for users who want to inspect or replace it.
pub const DEFAULT_PUNKT_BYTES: &[u8] = include_bytes!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/python/kaos_nlp_core/models/default.npkt.gz"
));

/// Build a `PunktSentenceTokenizer` from the embedded default model.
/// Panics only if the embedded bytes fail to decompress / deserialize,
/// which would indicate a corrupted build artifact rather than a runtime
/// error condition.
pub fn default_tokenizer() -> PunktSentenceTokenizer {
    let params = PunktParameters::from_compressed_bytes(DEFAULT_PUNKT_BYTES)
        .expect("embedded default Punkt bytes are valid");
    PunktSentenceTokenizer::from_parameters(Arc::new(params))
}
